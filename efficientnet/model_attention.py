...
class Channel_attention:
    ...
    def estimate_attention_map(self):
        ...
        shared_dense = tf.keras.Sequential([
                            ...
                            name=self.prefix + 'Channel_att_shared_dense')
        encoded_max_pool = shared_dense(max_pool)
        encoded_avg_pool = shared_dense(avg_pool)
        pooled_attention_info = encoded_max_pool + encoded_avg_pool
        attention_map = tf.keras.layers.Activation('relu',
                                                   name=self.prefix + 'Channel_att_act')(pooled_attention_info)
...
class Spatial_attention:
    ...
    def estimate_attention_map(self):
        input_for_pooling = tf.expand_dims(self.input_feature, axis=-1)
        max_pool = tf.keras.layers.MaxPool3D(
                        pool_size=(1, 1, self.num_channels), strides=1,
                        name=self.prefix + 'Spatial_att_max_pool')(input_for_pooling)
        avg_pool = tf.keras.layers.AveragePooling3D(
                        pool_size=(1, 1, self.num_channels), strides=1,
                        name=self.prefix + 'Spatial_att_avg_pool')(input_for_pooling)
        pooled_info = tf.keras.layers.Concatenate()([max_pool, avg_pool])
        pooled_info = tf.squeeze(pooled_info, axis=-2)
        encoded_info = tf.keras.layers.Conv2D(1, 7, padding="same",
                                              name=self.prefix + 'Spatial_att_conv2d')(pooled_info)
        encoded_info = tf.keras.layers.BatchNormalization(epsilon=1e-5, momentum=0.01,
                                                          name=self.prefix + 'Spatial_att_bn')(encoded_info)
        attention_map = tf.keras.layers.Activation('sigmoid',
                                                   name=self.prefix + 'Spatial_att_act')(encoded_info)
        return attention_map

    def attention_module(self, input_feature, prefix=''):
        self.input_feature = input_feature
        self.num_channels = input_feature.shape[-1]
        self.prefix = prefix
        attention_map = self.estimate_attention_map()
        attention_value = input_feature * tf.tile(
                            attention_map, [1, 1, 1, self.num_channels])
        return attention_value
